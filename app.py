...
def make_overlay(width, height, header, footer, font_size=12, margin_mm=8):
...
def annotate_pdf_web(pdf_bytes, xlsx_bytes, max_per_sheet):
    ...
    for i, _ in enumerate(reader.pages):
        ...
        # Wyciągamy kandydatów numerycznych (do statystyki i raportu)
        ...
        # --- priorytet: numer ZLECENIA z linii "Sales Order" ---
        sales_order = None
        for ln in page_text.splitlines():
            if "sales order" in ln.lower():
                nums_in_ln = re.findall(r"\d+", ln)
                if nums_in_ln:
                    sales_order = nums_in_ln[-1]
                break

        # fallback: gdy nie znaleziono linii "Sales Order", próbujemy nadal dopasować do Excela
        if not sales_order:
            sales_order = next((n for n in cands if n in excel_numbers), None)
